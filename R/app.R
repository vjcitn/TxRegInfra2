...
collect_tissue_options = function() {
  ...
  orgs = uu$name[unique(grep("^UB", TxRegInfra2::dnmeta$Biosample.term.id, value=TRUE))]
  cc = ontoProc::getCellOnto()
  cells = cc$name[unique(grep("^CL", TxRegInfra2::dnmeta$Biosample.term.id, value=TRUE))]
